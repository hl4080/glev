...
func main() {
	...
	events.OnOpened = func(c glev.Conn) (out []byte, opts glev.Options, action glev.Action) {
		c.SetContext(&glev.InputStream{})
		//log.Printf("opened: laddr: %v: raddr: %v", c.LocalAddr(), c.RemoteAddr())
		return
	}

	events.OnClosed = func(c glev.Conn, err error) (action glev.Action) {
	...
	// Start serving!
	log.Fatal(glev.Serve(events, addrs...))
}
